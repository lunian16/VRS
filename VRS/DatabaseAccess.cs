    ...
    public class DatabaseAccess
    {
        ...
        public void InsertRecordIfNotExists(int customerid, string lastname, string firstname, string contactphone, string email)
        {
            using (var connection = new MySqlConnection(BuilderString.ConnectionString))
            {
                ...
                // Proceed with insertion if the record does not exist
                string query = "INSERT INTO customer (last_name, first_name, contact_phone, email) value ( @last_name, @first_name, @contact_phone, @email)"; using (var command = new MySqlCommand(query, connection))
                {
        ...
        //******************************************//
        //manage rental
        public void InsertRecordIfNotExists(DateTime applydate, DateTime rental_date, DateTime return_date, double cost, int equipment_id, int customer_id)
        {
            using (var connection = new MySqlConnection(BuilderString.ConnectionString))
            {
                connection.Open();
                ...
                // Proceed with insertion if the record does not exist
                string query = "INSERT INTO rental (applydate, rental_date, return_date, cost,equipment_id,customer_id) value ( @applydate, @rental_date, @return_date, @cost, @equipment_id, @customer_id)"; using (var command = new MySqlCommand(query, connection))
                {
        ...
        public List<Rental> CompareDate()
        {
            ...
            {
                connection.Open();
                string sql = "SELECT rental_date, return_date, equipment_id FROM rental";
                MySqlCommand command = new MySqlCommand(sql, connection);
                using (MySqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        {
                            rentaldate = reader.GetDateTime(0),
